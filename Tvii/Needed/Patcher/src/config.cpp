...
namespace config {
  ...
  WUPSConfigAPICallbackStatus ConfigMenuOpenedCallback(WUPSConfigCategoryHandle rootHandle) {
    ...
    try {
      ...
      root.add(WUPSConfigItemBoolean::Create(CONNECT_TO_ROSE_CONFIG_ID, "Debug Tvii?", CONNECT_TO_ROSE_DEFUALT_VALUE, connectToRose, connectToRoseChanged));
      
      if (!utils::isJapanConsole()) {
        root.add(WUPSConfigItemStub::Create("-- Optional if have wuhb Tvii --"));
  ...
  void InitializeConfig() {
      // Add the config
    WUPSConfigAPIOptionsV1 configOptions = {.name = "Debug-Tvii"};
